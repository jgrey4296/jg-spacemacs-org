(when (configuration-layer/package-usedp 'org)
  ...
  (defun jg-spacemacs-org/open_link_in_buffer ()
    """ a util function to force links to be open in emacs  """
    (interactive)
    (org-open-at-point 'in-emacs)
    )
  (defun jg-spacemacs-org/open_link_externally ()
  ...
  (defun jg-spacemacs-org/change_link_name (name)
  ...
  (defun jg-spacemacs-org/list-agenda-files ()
...
(when (configuration-layer/package-usedp 'org-ref)
  ...
  (defun jg-spacemacs-org/org-ref-open-bibtex-pdf ()
...
(when (configuration-layer/package-usedp 'org-pomodoro)
  ...
  (defun jg-spacemacs-org/pomodoro-end-hook ()
    ;; create the temp buffer
    (progn
      (evil-window-new (get-buffer-window (current-buffer))
                       jg-spacemacs-org/pomodoro-buffer-name)
      (set (make-local-variable 'backup-inhibited) t)
      (auto-save-mode -1)
      (evil-window-set-height 10)
      (evil-initialize-local-keymaps)
      (evil-local-set-key 'normal (kbd "C-c C-c")
                          'jg-spacemacs-org/pomodoro-finish)
      (insert jg-spacemacs-org/pomodoro-log-message)
      (insert "Pomodoro Session: ")
      (redraw-display)
      )
    )
  (defun jg-spacemacs-org/pomodoro-finish ()
    ;; get the text
    (interactive)
    (let* ((text (buffer-substring (length jg-spacemacs-org/pomodoro-log-message) (point-max)))
